...
router.get('/overview', async (req, res) => {
  try {
    ...
    let projectFilter = {};
    if (userRole === 'partner') {
      projectFilter = { 'partner.id': { $in: req.user.dataScope.partners } };
    } else if (userRole !== 'admin') {
      projectFilter = { _id: { $in: req.user.dataScope.projects } };
    }
    ...
    if (userRole === 'dev' || userRole === 'qa') {
      ...
      taskFilter = { project: { $in: req.user.dataScope.projects } };
    }
    ...
    if (userRole === 'qa') {
      ...
      bugFilter = { project: { $in: req.user.dataScope.projects } };
    }
